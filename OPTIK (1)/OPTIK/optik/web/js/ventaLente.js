...
function cargarTablaClientes(data) {
    ...
    data.forEach((cliente, indice) => {
        ...
        contenido += `<td class='text-end'><button type='button' class='btn btn-outline-success m-3 ms-auto' onclick='getAllExamenVista(); getAllArmazon(); getAllTratamiento(); getAllMateriales(); getAllTipoMica();'>Agregar</button></td>`;
